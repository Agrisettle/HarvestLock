...
- [x] **Day 1 (1 Sept)** — `SPRINT.md`, `HANDOFF.md` (project-wide, new — distinct from `HarvestLock-Contracts/HANDOFF.md` which stays component-scoped), `CONTRIBUTING.md`. CI: contract tests + site build on every push/PR. First batch of scoped GitHub issues filed. API scaffold: package.json, Postgres schema, connection to the deployed testnet contract confirmed working.
- [ ] **Day 2** — API: contract lifecycle endpoints (create commitment, lock, get status, trigger claim/reclaim, settle) proxying to `HarvestLock-Contracts` via `@stellar/stellar-sdk`. Tests against testnet.
