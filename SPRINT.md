...
- [x] **Day 4 (started early, 1 Sept)** — `coop-pwa`: read-only dashboard hitting the API (contract status, claim windows). No write actions, matching the plan. "Allocation" display specifically **not** built — the allocation ledger itself doesn't exist yet (blocked on the salt-scheme decision, see `TASKS.md`), so there's nothing to show. Checked in a real browser, not just typecheck/build: caught a real CORS bug in the API (fixed same day, see `api/HANDOFF.md`).
- [x] **Day 5 (started early, 1 Sept)** — `buyer-app`: same read-only-first approach, buyer-facing framing (a plain-language "what's pending" summary instead of coop-pwa's raw tranche-first layout). Browser-verified against the live API and testnet; no new bugs found (the CORS fix from Day 4 already covers it). Days 2, 4, and 5 all landed the same day — Day 3 (allocation ledger) is genuinely next, still blocked on the salt-scheme decision.
