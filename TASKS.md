...
- [x] **Buyer default / side-selling forfeiture paths** — built 2 Sept 2026 as two-phase funding (`lock` now escrows only the deposit) plus `ready_for_delivery()`/`fund_remainder()`/`expire_remainder_window()`/`reclaim_on_nondelivery()`. Buyer default: `expire_remainder_window()`, permissionless, sweeps escrow to the cooperative, `Status::Defaulted` — per this session's product decision, carries an *immediate* off-chain permanent bar (not graduated), contingent on prior disclosure (see `site/` roles doc, still to be written) and an appeal channel (email, see below). Seller non-delivery: `reclaim_on_nondelivery()`, buyer-gated, returns escrow to the buyer, `Status::Forfeited` — carries a *graduated* 3-strike system before a bar, reflecting that the seller genuinely can't access escrowed funds unilaterally. Both are uncontested-by-construction deadline checks; `Status::Disputed` remains unbuilt (needs a real arbitration mechanism this session didn't design). 58/58 unit tests, three-scenario live-testnet verification (happy path, buyer-default sweep by an unrelated third-party signer, seller-non-delivery reclaim) — see `HarvestLock-Contracts/HANDOFF.md`. Off-chain reputation/strikes tracking, the appeals inbox (`samuelojetunde898@gmail.com`, user-supplied), the `buyer-app` consent checkbox, and the `site/` Roles & Responsibilities doc are all still open — see `api/` and `site/` sections below.
