...
| `coop-pwa/` | **Read-only dashboard real and browser-verified.** Commitment status + tranche claim windows, live from chain via the API. | React/Vite. Phone-auth, write actions, offline-tolerant queue all deferred — see `coop-pwa/README.md`. Do not build a logged-in app experience for individual farmers — PRD §13 (P3) and §16.1 rule that out; farmers get SMS only. |
| `buyer-app/` | **Read-only dashboard real and browser-verified.** Same data as `coop-pwa`, buyer-framed (leads with a plain-language "what's pending" summary). | Desktop-first web app. Write actions, auth, ERP integration (PRD §16.2) all deferred — see `buyer-app/README.md`. |
...
---
*Last updated: 1 Sept 2026, sprint day 1 — API's core lifecycle (deploy, initialize, lock, claim/reclaim, checkpoint, confirm, settle) built and verified end to end against live testnet via the actual HTTP server, not just the SDK wrapper. Postgres cache wired in. `coop-pwa`'s read-only dashboard built next, same day, and verified in a real browser — which caught a real CORS bug the HTTP-level API tests had missed entirely (`curl` doesn't enforce CORS). `buyer-app` followed the same day with the same read-only slice, buyer-framed, also browser-verified — no new bugs found there, since the CORS fix already covers it. CI now runs on every push/PR for site, api, coop-pwa, and buyer-app. Contracts gained `cancel()` (mutual unwind, PRD §7), which turned out to be genuinely instructive: it's the one method needing two different parties' Soroban auth on one call, and the obvious ways to do that (stellar-cli's `tx sign`, then the SDK's `Transaction.sign()` called twice) both fail the same way for the same reason. The correct mechanism is documented in `api/test/helpers.ts` and proven live with two real, different signers.*
