...
- **`oracle_config` wired into `initialize`, plus two new read routes** (5 Sept 2026): `HarvestLock-Contracts`' `initialize` gained a 13th, optional argument the same day (`Option<OracleConfig>` — Deployment 8, PRD §16.3's oracle staleness bound) that this API had not been updated to pass at all until now — a real gap `warehouse-app` surfaced (see `TASKS.md`), not one this repo introduced today. `InitializeBody`/`InitializeArgs` gained `oracleConfig?: { oracleContract, priceAsset, maxAgeSecs } | null`, validated the same way `contractedQuantity`/`gradePriceBps` are (mirror the contract's own guards, no extra business judgment). `deploy.ts`'s `oracleConfigToScVal` builds the struct's `ScVal::Map` by hand, same reasoning as `allocationMemberToScVal` — **and** the map's keys have to be in ascending Symbol order (`max_age_secs` < `oracle_contract` < `price_asset`), not struct-declaration order, since Soroban's host rejects an out-of-order `ScVal::Map` as malformed rather than just looking fields up by name regardless of position. `GET .../oracle-config` (`null` if never set — a valid `Ok(None)`, not an error, unlike `get_allocation`'s `AllocationNotSet`) and `GET .../oracle-rate` (a genuine live cross-contract call to the configured Reflector oracle on every request, never cached) round out the read side. Verified by hand against Deployment 8's real testnet WASM before writing the automated test: deploy → build `initialize` with a real Reflector oracle address (`CCSSOHTBL3LEWUCBBEB5NJFC2OKFRC74OWEIJIZLRJBGAAU4VMU5NV4W`, GBP) through the actual HTTP route → sign → submit → read both new routes back, config round-tripping exactly and the rate read genuinely live (a fresh timestamp on a second call, not a cached one). `ESCROW_WASM_HASH` bumped locally to Deployment 8's hash to run this (`.env` is gitignored, so this is a local-only change — the committed `.env.example` still has no hash baked in, same as before). **Updated 6 Sept 2026**: the contracts repo added `OracleConfig.denominated_amount` the very next day (Deployment 9, PRD §4.2 option (b) — see `HarvestLock-Contracts/HANDOFF.md`) to wire the FX-shortfall payout math into `settle`, and this API's oracle wiring — built the day before that field existed — had not been updated to match, a real gap caught before any caller could hit it in production rather than after. `InitializeOracleConfig`/`InitializeBody.oracleConfig` gained `denominatedAmount`; `oracleConfigToScVal`'s hand-built map gained the field in its correct alphabetical position (`denominated_amount` sorts before `max_age_secs`); `GET .../oracle-config` now returns it too. Also fixed the same staleness one level up: `client.ts`'s `Commitment` interface (and `warehouse-app/src/api.ts`'s `CommitmentDetail` copy) never gained the 5 FX fields (`fx_resolved`, `fx_adjusted_total`, `fx_shortfall_amount`, `fx_shortfall_funded`, `fx_shortfall_deadline`) the contract's `Commitment` struct gained in the same commit — `serializeCommitment` now stringifies the two new bigints too. 2 new validation tests; `ESCROW_WASM_HASH` re-bumped to Deployment 9 and the full 4-field flow re-verified live through the real HTTP layer.
...
- **Dispute flagging — `flag_dispute`, `resolve_dispute`, `expire_dispute_window`** (6 Sept 2026, `HarvestLock-Contracts` Deployment 10): closes PRD's must-have "dispute flagging with defined escalation," a gap since the very first version of the contract. `flag_dispute` takes an argument (`flagger`, whichever of buyer/cooperative/warehouse_operator is flagging) and needs only that one party's auth, so — same reasoning as `confirm_delivery`/`reassign_buyer` — it got its own route (`POST .../tx/flag-dispute`) rather than the generic no-arg one; the API doesn't cross-check `flagger` against the commitment's own fields (same minimal-validation convention as `confirm_delivery` not checking `sourcePublicKey` against `warehouse_operator`) since lib.rs's own `NotAParty` error is the actual enforcement. `resolve_dispute` needs all three parties' auth, same shape as `cancel`/`reassign_buyer`: it's in the no-arg method set (sign the same envelope three times, already generically supported) **and** has its own staged `propose`/`sign` route pair (`POST .../tx/resolve-dispute/propose`) for the separate-wallets case — `buildMultiPartyProposal` needed zero changes, since simulating `resolve_dispute` naturally produces three non-source auth entries the same way `reassign_buyer`'s simulation already does for its own three. `expire_dispute_window` is permissionless and no-arg, straight into the existing set next to `expire_remainder_window`. `client.ts`'s `Commitment` gained `dispute_pre_status`/`dispute_deadline` (the former unwrapped through the same `unwrapStatus` helper `status` already uses — `Status` decodes to a one-element array, not a bare string); `serializeCommitment` stringifies the new bigint. Live-verified end to end through the real HTTP layer (`test/stellar.test.ts`): cooperative flags via the real route → status genuinely `Disputed` → a subsequent `release_advance_1` call rejected → an unrelated third party's resolve-propose rejected (403) → warehouse operator proposes → buyer and cooperative each sign their own entry → finalize → status genuinely back to `Locked`, and a fresh `release_advance_1` call succeeds, proving the un-freeze is real. 5 new network-free validation tests in `server.test.ts`; `ESCROW_WASM_HASH` bumped to Deployment 10's hash.
- **Explicit `setErrorHandler` + rate limiting** (9 Sept 2026, self-audit finding): `errors.ts`'s `HttpError` doc comment used to say "see server.ts's setErrorHandler" while no such handler actually existed anywhere in the codebase — a real, checked gap, not a hypothetical: it meant every unhandled exception's raw `.message` reached the HTTP response via Fastify's own default handler, unreviewed. Fixed by actually adding one. It reproduces Fastify's default `{statusCode, error, message}` shape exactly (existing tests read `.json().message`, not `.json().error` — a first pass at this got that backwards and briefly broke ~50 tests, now caught and fixed) for two categories: `HttpError` subclasses (`BadRequestError`/`ForbiddenError`, unchanged client-facing behavior) and the new `ConfigurationError` (a missing `DEPLOYER_SECRET_KEY`/`ESCROW_WASM_HASH`/`STELLAR_RPC_URL` — an operator mistake, given a generic message instead of naming the specific env var externally). Everything else — Stellar simulation/submission/timeout failures, SDK decoding assertions — deliberately still propagates with its real message and status code, unchanged from before this handler existed: see `stellar/client.ts`'s "let the contract error propagate" convention and every build/submit route's simulation-failure feedback, which callers genuinely need to build a working transaction; blanket-sanitizing every 500 would have silently broken that. Checked empirically before deciding sanitization was even needed for the config-error case specifically: `Keypair.fromSecret()` never echoes an invalid value back in its error message, and a real `pg` connection-refused failure (tested against a deliberately wrong `connectionString`) came back with an *empty* `.message`, not connection detail — so there was never a proven secret-leak path, this is defense-in-depth plus a genuinely false comment fixed, not a patched active vulnerability. `@fastify/rate-limit` also added (100 req/min per IP, global, engineering default not a researched value, same caveat this file's window bounds already carry) — nothing bounded request volume at all before this. Both disabled under `NODE_ENV=test` (verified empirically that vitest sets this by default in this project, not assumed) so the existing suites' rapid-fire requests against one shared `app` instance don't trip the limiter. Verified: typecheck clean, `npm run build` clean, and every one of the ~50 tests that don't require a live local Postgres (which happened to be down in this environment at the time) pass with the corrected response shape — the DB-dependent failures were checked individually and are 100% `ECONNREFUSED`, zero unrelated assertion mismatches, so this wasn't verified against the full live-DB suite in this pass; worth a full `npm test` run once Postgres is back up.
...
dispute-flagging methods (`flag_dispute`/`resolve_dispute`/
`expire_dispute_window`, `HarvestLock-Contracts` Deployment 10): a new
single-signer route for `flag_dispute`, `resolve_dispute` added to both
the no-arg method set and its own staged propose/sign route pair
(needed zero changes to the underlying multi-party machinery — see
"What's real" above), `expire_dispute_window` added to the no-arg set.
Live-verified end to end through the real HTTP layer: flag → freeze
confirmed by a rejected call → an unrelated party's resolve-propose
rejected → real three-party propose/sign/finalize → status restored →
normal processing resumed. This closes PRD's must-have "dispute
flagging with defined escalation," a gap since the contract's very
first version — see `HarvestLock-Contracts/HANDOFF.md`'s Deployment 10
for the contract-level detail and its own direct-`stellar-cli`
verification of `flag_dispute`/`expire_dispute_window`.

Prior entry (6 Sept 2026): the contracts repo added `OracleConfig.
denominated_amount` (Deployment 9, PRD §4.2 option (b)'s FX shortfall
payout wiring) the day after this API's `oracle_config` wiring shipped
against the 3-field version; updated `oracleConfigToScVal`, the
`InitializeOracleConfig`/`InitializeBody` types, the `GET .../oracle-config`
response, and the `Commitment`/`CommitmentDetail` TS interfaces (which had
also missed the contract's 5 new FX fields) to match. Re-verified live
against Deployment 9 through the real HTTP layer. See "What's real" above
for the full writeup.

Prior entry (5 Sept 2026): wired the contract's new `oracle_config`
