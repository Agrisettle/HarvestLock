...
| POST | `/commitments/:contractId/tx/:method` | Builds unsigned XDR for any no-argument lifecycle method (`lock`, `release_advance_1/2`, `claim_advance_1/2`, `reclaim_advance_1/2`, `mark_checkpoint`, `confirm_delivery`, `settle`). |
...
throwaway contract instance, builds and submits a real `initialize`
transaction, and asserts on the resulting on-chain state. This costs real
(testnet) transaction submissions each run — that's the point, it's the
only way to know the SDK usage is actually correct.
