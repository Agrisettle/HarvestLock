 ...
 *   same shape as this app's existing `mark_checkpoint`/`confirm_delivery`
 *   actions in `CommitmentDetail.tsx` (there's nothing to stage — one
 *   signature is the whole transaction).
 * - **Resolving** (`Status === "Disputed"`) needs all *three* parties'
 *   auth, so it reuses buyer-app/coop-pwa's proven staged propose/sign/
 *   finalize mechanism (`ReassignBuyerSection.tsx` there, for two pending
 *   signers) — genuinely new machinery for *this* app, though, since its
 *   original two actions are both single-signer and it never had a
 *   Cancel/ReassignBuyer-equivalent component to generalize from before
 *   now. `wallet.ts` gained `signAuthEntry` for exactly this. Same
 *   `justApproved` local-state gap as the other two apps' copies: once a
 *   signer's entry is recorded, the API's `pending_entries` stops naming
 *   them, so this component can't tell purely from a page reload that
 *   "you already signed."
 *
 * Checks all *three* named parties (buyer, cooperative, warehouse
 * operator), not just the warehouse operator this app is nominally
 * built for — nothing stops the buyer's or cooperative's wallet from
 * connecting here if they have the link, and they should be able to act
 * exactly as the contract's own auth model allows.
 *
 * Identical to buyer-app/coop-pwa's copy of this file — same
 * small-duplication call as `wallet.ts`/`api.ts` across the three apps,
